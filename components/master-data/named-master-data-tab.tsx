...
import { upsertNamedItemAction, type NamedMasterTable } from "@/lib/actions/master-data";

type Row = { id: string; name: string; is_active: boolean; display_order: number };

export function NamedMasterDataTab({ table, rows, canEdit }: { table: NamedMasterTable; rows: Row[]; canEdit: boolean }) {
  ...
  async function handleSave(id: string | null, name: string, isActive: boolean, displayOrder: number) {
    setPending(true);
    setError(null);
    const result = await upsertNamedItemAction(table, id, name, isActive, displayOrder);
    setPending(false);
    if (result.error) {
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-surface">
        <table className="w-full text-sm">
          ...
          <tbody>
            {adding && canEdit && (
              <InlineEditRow row={null} onCancel={() => setAdding(false)} onSave={handleSave} pending={pending} />
            )}
            {rows.map((r) =>
              editingId === r.id ? (
                <InlineEditRow key={r.id} row={r} onCancel={() => setEditingId(null)} onSave={handleSave} pending={pending} />
              ) : (
            ...
            {rows.length === 0 && !adding && (
              <tr>
                <td colSpan={canEdit ? 4 : 3} className="px-4 py-8 text-center text-sm text-ink-muted">
                  No entries yet.
                </td>
              </tr>
            )}
...
function InlineEditRow({
  row, onCancel, onSave, pending,
}: {
  row: Row | null;
  onCancel: () => void;
  onSave: (id: string | null, name: string, isActive: boolean, displayOrder: number) => void;
  pending: boolean;
}) {
    ...
    <tr className="border-b border-border bg-primary-light/20 last:border-0">
      ...
      <td className="px-4 py-2">
        <div className="flex gap-2">
          <button
            onClick={() => onSave(row?.id ?? null, name, isActive, displayOrder)}
            disabled={pending}
