...
export async function checkLiveFlightStatusAction(flightNumber: string): Promise<LiveFlightStatusResult> {
  ...
  // Read the body regardless of response.ok — AviationStack returns a
  // real, useful JSON error body (with a code and message) even for
  // 401/403/404/429 responses, per their documented error format. The
  // old version discarded this entirely on any non-2xx status, showing
  // a generic "didn't respond" message no matter what actually went
  // wrong (bad key, wrong plan, rate limit, etc.) — exactly the kind of
  // unhelpful error this project has been fixing everywhere else.
  let json: unknown;
  try {
    json = await response.json();
  } catch {
    return { error: `The flight status service returned an unexpected response (HTTP ${response.status}).` };
  }

  const body = json as { error?: { code?: string; message?: string }; data?: unknown };

  if (body.error) {
    return { error: body.error.message || `The flight status service reported an error (${body.error.code ?? response.status}).` };
  }
  if (!response.ok) {
    return { error: `The flight status service returned an unexpected error (HTTP ${response.status}).` };
  }
