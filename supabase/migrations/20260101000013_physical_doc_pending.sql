-- ============================================================
-- Last piece of the "Pending" consistency work: physical_doc_status was
-- deliberately left at 'Originals Pending' (it already contained the
-- word "Pending" and was more specific) — explicitly overridden now on
-- direct instruction: every module should show the literal word
-- "Pending" as its default, full stop, with 'Not Required'/'Not
-- Applicable' still selectable afterward but never the starting state.
--
-- Same idempotent pattern as the previous fix — checks before renaming,
-- safe to run regardless of whether this already partially landed.
-- ============================================================
...
begin
  if exists (
    ...
    where t.typname = 'physical_doc_status' and e.enumlabel = 'Originals Pending'
  ) then
    alter type physical_doc_status rename value 'Originals Pending' to 'Pending';
  end if;
end $$;

alter table shipments alter column physical_doc_status set default 'Pending';

select
  (select exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid where t.typname='physical_doc_status' and e.enumlabel='Pending')) as physical_doc_status_has_pending,
  (select column_default from information_schema.columns where table_name='shipments' and column_name='physical_doc_status') as physical_doc_status_default;

-- search_shipments: the 'physpending' saved-view filter compared against
-- the literal string 'Originals Pending'.
...
-- get_dashboard_metrics: the physical_docs_pending KPI count and the
-- "Physical documents not dispatched" alert both compared against
-- 'Originals Pending'.
