-- ============================================================
-- IDEMPOTENT FOLLOW-UP to 20260101000011_status_pending_default.sql.
--
-- That migration's "ERROR: 22023: 'Not Started' is not an existing enum
-- label" means at least one of the three renames it attempted already
-- happened somehow before this ran — most likely a prior partial
-- application. Rather than guess at the exact prior state, every rename
-- below is wrapped in a check against pg_enum first, so this is safe to
-- run no matter which of the three (if any) already went through.
--
-- Everything else from migration 11 (the function replacements, the
-- mofaic_status default change) is naturally safe to re-run regardless —
-- `create or replace function` and `set default` don't error on repeat —
-- so those are simply repeated here rather than requiring you to
-- figure out which parts already landed.
-- ============================================================
...
begin
  if exists (
    ...
    where t.typname = 'document_status' and e.enumlabel = 'Not Started'
  ) then
...
begin
  if exists (
    select 1 from pg_enum e join pg_type t on t.oid = e.enumtypid
    where t.typname = 'customs_status' and e.enumlabel = 'Not Started'
  ) then
...
begin
  if exists (
    select 1 from pg_enum e join pg_type t on t.oid = e.enumtypid
    where t.typname = 'municipality_status' and e.enumlabel = 'Not Started'
  ) then
...
-- Confirms the actual current state after the above, so the SQL Editor's
-- own output tells you definitively whether every column is where it
-- should be — no need to separately query the register to check.
select
  (select exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid where t.typname='document_status' and e.enumlabel='Pending')) as document_status_has_pending,
  (select exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid where t.typname='customs_status' and e.enumlabel='Pending')) as customs_status_has_pending,
  (select exists(select 1 from pg_enum e join pg_type t on t.oid=e.enumtypid where t.typname='municipality_status' and e.enumlabel='Pending')) as municipality_status_has_pending,
  (select column_default from information_schema.columns where table_name='shipments' and column_name='mofaic_status') as mofaic_status_default;
