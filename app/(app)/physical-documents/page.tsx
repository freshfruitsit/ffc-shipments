...
export default async function PhysicalDocumentsWorkspacePage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-xl font-semibold text-ink">Physical Documents</h1>
        <p className="text-sm text-ink-muted">
          Original documents still awaiting dispatch or in transit via Zajeel.
        </p>
      </div>

      <RegisterFilterBar />
      <p className="text-xs text-ink-muted">{totalCount} shipment(s) with originals pending</p>

      <WorkspaceTable
        results={results as WorkspaceShipmentRow[] | null}
        error={!!error}
        focus="physicalDoc"
        emptyMessage="No original documents are pending dispatch right now."
      />
